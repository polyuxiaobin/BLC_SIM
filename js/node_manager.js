...
function addNewInspectionPrompt() {
    $('#new_inspection_window').modal("show");
    try {
        var route_input = ($('#new_inspect_route_input'));
    ...
    var detectInput = function () {
        var name = $('#new_inspect_name_input').val().toString().trim();
        var route = $('#new_inspect_route_input').val().toString().trim();
        if (name && route && route.startsWith('/')) {
    ...
    $('#new_inspect_add_btn').off('click');
    $('#new_inspect_add_btn').on('click', function (event) {
        var name = $('#new_inspect_name_input').val().toString().trim();
        var route = $('#new_inspect_route_input').val().toString().trim();
        console.log(name, route);
        addNewInspect(name, route);
        var custom_key = 'custom_inspects';
        if (!(custom_key in Object.keys(states_1["default"]))) {
            states_1["default"]['custom_inspects'] = {};
        }
        states_1["default"]['custom_inspects'][name] = route;
        $('#new_inspection_window').modal('hide');
        setupCustomInspectionEvents();
    });
}
exports.addNewInspectionPrompt = addNewInspectionPrompt;
function setupCustomInspectionEvents() {
    $('.custom_inspect_menu_item').off('click');
    $('.custom_inspect_menu_item').on('click', function (event) {
        var item = $(event.target);
        var href = item.attr('href');
        var itemContentDiv = $(href);
        var name = item.attr('name');
        var route = item.attr('route');
        var id = item.attr('nodeOrder');
        var ip = $("#collapse_" + id).find('.ip_input').val();
        var base = utils_1["default"].convertToURL(ip);
        var routeURL = utils_1["default"].joinURL(base, route);
        itemContentDiv.html("Reloading...");
        connector_1["default"].getResponse(routeURL, null, function (resp) {
            //console.log('resp=',resp);
            var div = (itemContentDiv);
            div.jsonViewer(resp, { collapsed: true, rootCollapsable: false });
        }, function (error) {
            //console.log("error=",error);
            var innerHTML = generateInspectContent(name, routeURL);
            itemContentDiv.html(innerHTML);
        });
    });
}
exports.setupCustomInspectionEvents = setupCustomInspectionEvents;
function generateInspectContent(name, route) {
    return "<div>" + name + " -- " + route + "</div>";
}
function addNewInspect(name, route) {
    var inspection_windows = $('.custom_inspection');
    $.each(inspection_windows, function (index, inspection_div) {
        var list_group_id = $(inspection_div).closest('.list-group').attr('id');
        var id = list_group_id.replace('list-tab-', '');
        var menuItem = "<a class=\"list-group-item list-group-item-action custom_inspect_menu_item\" name=\"" + name + "\"  route=\"" + route + "\" nodeOrder=\"" + id + "\"  data-toggle=\"list\" href=\"#list-custom_" + name + "_" + id + "\" role=\"tab\" aria-controls=\"addinspect_" + name + "_" + id + "\"><i class=\"fas fa-sync-alt\" style=\"padding-right:5px;\"></i>" + name + "</a>";
        //let group_list = $(`#list-group_content_${id}`);
        var ip = $("#collapse_" + id).find('.ip_input').val();
        //let inspectContent = generateInspectContent(ip);
        var menuItemContent = "<div class=\"tab-pane fade\" id=\"list-custom_" + name + "_" + id + "\" role=\"tabpanel\" aria-labelledby=\"list-addinspect-list-" + id + "\">Loading...</div>";
