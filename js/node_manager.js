...
function setupLaunchBehaviors() {
    ...
    $('.launchBtn').on('click', function (event) {
        ...
        var txRandomDelay = div.find('.txRandomDelay_input').val();
        ...
        if (!txRandomDelay) {
            ...
            txRandomDelay = parseFloat(txRandomDelay);
        }
        ...
        var miningURL = utils_1["default"].joinURL(url, mineRoute);
        ...
        var txString = txURL + "-" + txGenerateInterval + "-" + txRandomDelay + "-" + stopOnError;
        ...
        if (isRunning.trim() == "true") {
            ...
            if (!mineInstance || mineInstance.toString() != mineString) {
                miningInstanceMap[selector] = new mine_controller_1.MiningInstance(miningURL, true, node_name, node_order);
                mineInstance = miningInstanceMap[selector];
            }
            if (!txInstance || txInstance.toString() != txString) {
                transactionInstanceMap[selector] = new transaction_controller_1.TransactionInstance(txURL, txGenerateInterval, txRandomDelay, true, node_name, node_order);
...
function generateNodes(num, from, append, animated) {
    ...
    if (append) {
        var origin_1 = $('#generated').html();
        originalNodeNumber = $('#generated').find('.card').length;
        $('#generated').html(origin_1 + html);
    }
    else {
        $('#generated').html(html);
    }
